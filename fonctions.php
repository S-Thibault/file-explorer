...
function infofichier($chemin,$data) {
  ...
  $tableau['perms'] = mfunGetPerms(fileperms($data."/".$chemin));
  $t = explode(".", $chemin);
...
if ($dir = opendir($repertoire))
	...
	foreach($fichier as $affichage)
		{
		echo '<tr>';
		//echo '<td><a style="font-size: 10pt" href='.$_SERVER['HTTP_HOST'].'?type=dir&entry='.rawurlencode(str_replace($data.'/', "", $repertoire.'/'.$affichage['name'])).'><img src="images/file-none.gif" border=0 />'.$affichage['name'].'</a></td>';
